...
@async_test
async def test_red_line_animation_direction():
    """Verify red line animation trail appears on correct side based on movement direction."""
    # Setup
    game, _, _ = await create_test_game(visual=False)
    letter_source = game.letter_source
    window = pygame.Surface((800, 600))
    now_ms = 0
    
    initial_base_y = letter_source.initial_y
    ...
    letter_source.update(window, now_ms)
    
    # Verify State
    assert letter_source.moving_down is True, "Should be moving down"
    assert letter_source.height > LetterSource.MIN_HEIGHT, "Should have animation trail"
    ...
    expected_y = initial_base_y + game.letter.start_fall_y - letter_source.height
    assert letter_source.pos[1] == expected_y, "Trail should be above line on descent"

    # Reset animation state by letting it finish (fast forward time)
    now_ms += 2000
    letter_source.update(window, now_ms)
    assert letter_source.height == LetterSource.MIN_HEIGHT
    ...
    # Verify State
    assert letter_source.moving_down is False, "Should be moving up"
    assert letter_source.height > LetterSource.MIN_HEIGHT, "Should have animation trail"
    ...
    assert letter_source.pos[1] == expected_y_up, "Trail should be below line on ascent"
